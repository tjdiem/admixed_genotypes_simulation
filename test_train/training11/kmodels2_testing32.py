...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.conv = nn.Conv2d(1, 4, (self.conv_size, 1), padding="same")
    ...
    def forward(self, ref_sim):
        ...
        ref_sim = ref_sim.reshape(-1, 1, input_size, n_embd_model)
        ref_sim = self.conv(ref_sim)
        ref_sim = ref_sim.amax(dim=-3)
        ref_sim = ref_sim.reshape(-1, num_classes, n_ind_max, input_size, n_embd_model)
