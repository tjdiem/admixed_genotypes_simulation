...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.linear2a = nn.Linear(hidden2, hidden2a)
        self.ln2a = nn.LayerNorm(hidden2a)

        self.linear3 = nn.Linear(hidden2a, 1)
        ...
        self.linear1e = nn.Linear((num_classes * n_ind_pan_model),(num_classes * n_ind_pan_model))
        self.ln1e = nn.LayerNorm(num_classes * n_ind_pan_model)
    ...
    def forward(self, ref_sim):
        ...
        ref_sim = self.linear2a(ref_sim)
        ref_sim = self.ln2a(ref_sim)
        # RELU

        ref_sim = ref_sim.transpose(-2, -1)
        ref_sim = self.linear1e(ref_sim)
        ref_sim = self.ln1e(ref_sim)
        ref_sim = ref_sim.transpose(-2, -1)
