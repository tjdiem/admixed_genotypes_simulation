...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.linear2_2 = nn.Linear(hidden1, hidden2) 

        self.ln2 = nn.LayerNorm(hidden2)
        self.ln2_2 = nn.LayerNorm(hidden2)


        self.linear3 = nn.Linear(2 * hidden2, 1)
    ...
    def forward(self, ref_sim):
        # testing19 with concatenation residual  connection (both going through different linear2)
        ...
        ref_sim2 = ref_sim.transpose(-2, -1)
        ref_sim2 = self.linear1c(ref_sim2)
        ref_sim2 = self.ln1c(ref_sim2)
        ref_sim2 = ref_sim2.transpose(-2, -1)

        ref_sim2 = self.linear2_2(ref_sim2)
        ref_sim2 = self.ln2_2(ref_sim2)
        # RELU

        ref_sim2 = ref_sim2.transpose(-2, -1)
        ref_sim2 = self.linear1d(ref_sim2)
        ref_sim2 = self.ln1d(ref_sim2)
        ref_sim2 = ref_sim2.transpose(-2, -1)

        ref_sim = self.linear2(ref_sim)
        ref_sim = self.ln2(ref_sim)
        ref_sim = torch.cat((ref_sim, ref_sim2), dim=-1)
        ref_sim = self.relu(ref_sim)
