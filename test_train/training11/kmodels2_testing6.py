...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.linear1 = nn.Linear(input_size - self.conv_size + 1, hidden1)
        ...
        self.conv = nn.Conv2d(1, 1, (self.conv_size, hidden0))
    ...
    def forward(self, ref_sim):
        ...
        # output_tensor = torch.empty(16, 3, 48, input_size - self.conv_size + 1, 1).to(device)
        # for i in range(ref_sim.shape[0]):
        #     for j in range(ref_sim.shape[1]):
        #         for k in range(ref_sim.shape[2]):
        #             output_tensor[i,j,k] = self.conv(ref_sim[i,j,k].unsqueeze(0))

        output_tensor = self.conv(ref_sim.reshape(-1, 1, input_size, n_embd_model))
        output_tensor = output_tensor.reshape(-1, 3, 48, input_size - self.conv_size + 1, 1)

        # print(torch.equal(output_tensor, output_tensor))

        # final classification layers
        # ref_sim = self.block(ref_sim) ###
        output_tensor = output_tensor.reshape(*output_tensor.shape[:3], -1) # batch, num_classes, n_ind_max, input_size * hidden0
        output_tensor = self.linear1(output_tensor)
        output_tensor = self.relu(output_tensor)

        output_tensor = output_tensor.transpose(-2, -1)
        output_tensor = self.linear1c(output_tensor)
        output_tensor = output_tensor.transpose(-2, -1)

        output_tensor = self.linear2(output_tensor)
        output_tensor = self.relu(output_tensor)
        output_tensor = self.linear3(output_tensor) # batch, num_classes, num_classes * n_ind_pan_model
        ...
        output_tensor, _ = torch.sort(output_tensor, dim=2) 
        output_tensor = self.sigmoid(output_tensor).squeeze(-1) # batch, num_classes, num_classes * n_ind_pan_model
        output_tensor = self.linear4(output_tensor)
        output_tensor = self.relu(output_tensor)
        output_tensor = self.linear5(output_tensor) # batch, num_classes, 1
