...
if human_data:
    ...
    # Training parameters
    panel_template_dir = "../../data_simulated/panel_templates14/"
    panel_dir = "../../data_simulated/panels14/"
    split_dir = "../../data_simulated/splits14/"
    parameters_dir = "../../data_simulated/parameters14/"

if sys.argv[0] in ["train.py", "deep_clustering.py", "train_positions.py", "train_full_seq.py", "train_full_seq_copy.py", "train_var_IS.py", "train_NG.py"] or sys.argv[0].startswith("predict_cluster") or sys.argv[0].startswith("predict_full_seq"): #change this later
    num_files = 367
    train_prop = 0.9
    num_epochs = 2500
    batch_size = 16
...
elif sys.argv[0].startswith("train_cluster"):
    num_files = 270
    train_prop = 0.9
    num_epochs = 3000
    batch_size = 16
    eval_interval = 50   #how many epochs to train before evaluating model
    num_estimate = 1e10   #number of examples to estimate accuracy with
    lr_start = 3e-5
    lr_end = lr_start/100
    mixed_precision = False #train with mixed floating point precision?  helps with GPU memory issues
    checkpointing = False
...
# Processing parameters
if sys.argv[0] == "deep_clustering.py" or sys.argv[0].startswith("predict_cluster"):
...
elif sys.argv[0] in ["train_full_seq.py", "train_full_seq_copy.py", "train_NG.py"] or sys.argv[0].startswith("predict_full_seq"):
    num_bp = 50_000_000                           # total number of base pairs
    input_size_processing = "all"                 # total number of SNPs in processing
    input_size_step = 1                           # step between SNPs
    n_ind_adm_start = 0
