...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.linear2c = nn.Linear((num_classes * n_ind_pan_model),(num_classes * n_ind_pan_model))
        self.linear3 = nn.Linear(hidden1, hidden1)
        self.linear3c = nn.Linear((num_classes * n_ind_pan_model),(num_classes * n_ind_pan_model))
       
        self.linear2old = nn.Linear(hidden1, hidden2) 
        self.linear3old = nn.Linear(hidden2, 1)
    ...
    def forward(self, ref_sim):
        ...
        # testing10 with residual connections
        ...
        ref_sim = ref_sim + self.linear1c(ref_sim)
        ref_sim = ref_sim.transpose(-2, -1)
        ...
        # ref_sim = ref_sim.transpose(-2, -1)
        # ref_sim = self.linear2c(ref_sim)
        # ref_sim = ref_sim.transpose(-2, -1)

        ref_sim = ref_sim + self.linear3(ref_sim)
        ref_sim = self.relu(ref_sim)

        ref_sim = ref_sim.transpose(-2, -1)
        ref_sim = ref_sim + self.linear3c(ref_sim)
        ref_sim = ref_sim.transpose(-2, -1)

        ref_sim = self.linear2old(ref_sim)
        ref_sim = self.relu(ref_sim)
        ref_sim = self.linear3old(ref_sim) # batch, num_classes, num_classes * n_ind_pan_model
