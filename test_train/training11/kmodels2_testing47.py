...
class AttentionSetModel(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(AttentionSetModel, self).__init__()
        self.attention_weights = nn.Parameter(torch.randn(input_dim))  # Learnable attention weights
    ...
    def forward(self, x):
        ...
        attention_weights = torch.softmax(attention_scores, dim=-2)  # Normalize attention scores
        weighted_sum = torch.sum(x * attention_weights.unsqueeze(-1), dim=-2)  # Weighted sum of inputs
...
class ParallelAttentionSetModel(nn.Module):
    def __init__(self, input_dim, output_dim, num_models):
        ...
        # Create multiple AttentionSetModel instances
        self.attention_models = nn.ModuleList([
            AttentionSetModel(input_dim, output_dim) for _ in range(num_models)
        ])
        
        # A final fully connected layer to combine the outputs
        # self.fc = nn.Linear(output_dim * num_models, output_dim)

    def forward(self, x):
        # Apply each AttentionSetModel in parallel
        outputs = [model(x) for model in self.attention_models]
        
        # Concatenate the outputs along the last dimension (input_dim)
        output = torch.stack(outputs, dim=-2)  # Shape: (batch_size, num_points, output_dim * num_models)
        
        # Apply the final fully connected layer
        # output = self.fc(combined_output)  # Shape: (batch_size, num_points, output_dim)
...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.linear3 = nn.Linear(hidden2, hidden2_1)
        ...
        self.linear6 = nn.Linear(hidden2_1, 1)
        ...
        self.parallel_attention_set_model1 = ParallelAttentionSetModel(hidden1, hidden1, n_ind_max)
        self.parallel_attention_set_model2 = ParallelAttentionSetModel(hidden2, hidden2, n_ind_max)
        self.parallel_attention_set_model3 = ParallelAttentionSetModel(hidden2_1, hidden2_1, hidden3)
        self.parallel_attention_set_model4 = ParallelAttentionSetModel(hidden2_1, hidden2_1, 1)
    ...
    def forward(self, ref_sim):
        ...
        ref_sim = self.parallel_attention_set_model1(ref_sim)
        ...
        # print(ref_sim.shape)
        ref_sim = self.parallel_attention_set_model2(ref_sim)
        # print(ref_sim.shape)
        # print()
        ...
        ref_sim = self.parallel_attention_set_model3(ref_sim)
        ...
        ref_sim = ref_sim.squeeze()
        ref_sim = self.linear6(ref_sim).squeeze()
