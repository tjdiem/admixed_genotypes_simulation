...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.linear1c = nn.Linear((5 * num_classes * n_ind_pan_model),(num_classes * n_ind_pan_model))
    ...
    def forward(self, ref_sim):
        ...
        ref_sim_copy = torch.zeros((*ref_sim.shape[:-1], 5, ref_sim.shape[-1])).to(device)
        ref_sim_copy[..., 0, 2:] = ref_sim[..., :-2]
        ref_sim_copy[..., 1, 1:] = ref_sim[..., :-1]
        ref_sim_copy[..., 2, :] = ref_sim
        ref_sim_copy[..., 3, :-1] = ref_sim[..., 1:]
        ref_sim_copy[..., 4, :-2] = ref_sim[..., 2:]

        ref_sim_copy = ref_sim_copy.reshape((*ref_sim_copy.shape[:-2], -1))

        ref_sim = self.linear1c(ref_sim_copy)
        ref_sim = ref_sim.transpose(-2, -1)
