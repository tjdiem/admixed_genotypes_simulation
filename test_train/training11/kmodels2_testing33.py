...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.bilstm = nn.LSTM(hidden0, hidden0, num_layers=3, 
                               bidirectional=True, batch_first=True)

        self.linear1 = nn.Linear(2 * input_size * hidden0, hidden1)
    ...
    def forward(self, ref_sim):
        ...
        ref_sim = ref_sim.reshape(-1, ref_sim.shape[-2], ref_sim.shape[-1])
        ref_sim, _ = self.bilstm(ref_sim)
        ref_sim = ref_sim.reshape(-1, 3, 48, input_size, 2 * n_embd_model)
