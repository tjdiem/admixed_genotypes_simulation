...
class AttentionSetModelSummed(nn.Module):
    def __init__(self, input_dim, output_dim):
        super(AttentionSetModelSummed, self).__init__()
...
class AttentionSetModel(nn.Module):
    ...
    def forward(self, x):
        ...
        attention_scores = torch.matmul(x, self.attention_weights)  # Compute attention scores
        attention_weights = torch.softmax(attention_scores, dim=-2)  # Normalize attention scores
        weighted_sum = x * attention_weights.unsqueeze(-1)  # Weighted sum of inputs
        return self.fc(weighted_sum)
...
class KNet4(nn.Module):
    def __init__(self):
        ...
        self.attention_set_model1 = AttentionSetModel(hidden1, hidden2)
        self.attention_set_model2 = AttentionSetModel(hidden2, 1)
        self.attention_set_model_summed = AttentionSetModelSummed(hidden2, hidden2)
    ...
    def forward(self, ref_sim):
        ...
        ref_sim = self.attention_set_model1(ref_sim)
        ...
        ref_sim = self.selu(ref_sim)
        # RELU

        # print(ref_sim.shape)
        ref_sim = self.attention_set_model2(ref_sim)
        # print(ref_sim.shape)
        # print()
