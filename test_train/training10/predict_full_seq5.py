...
def predict_full_sequence(model, SOI, positions, refs, labels, batch_size=batch_size, num_generations=None, population_size=None):
    ...
    for iteration in range(num_iterations):
        ...
        if True:#iteration == num_iterations - 1:
            ...
            for i in range(1, outputs_predicted.shape[0]):
                # corr = (transitions[i - 1][0, 0] - 1/4) * (4/3)
                corr = e ** (-20 * num_generations * positions_diff[i - 1])
                prob_weights_left[i] = corr * prob_weights_left[i - 1] + 1


            prob_weights_right = torch.zeros((outputs_predicted.shape[0],)).to(device).float()
            prob_weights_right[-1] = 1
            for i in range(outputs_predicted.shape[0] - 2, -1, -1):
                # corr = (transitions[i][0, 0] - 1/4) * (4/3)
                corr = e ** (-20 * num_generations * positions_diff[i])
                prob_weights_right[i] = corr * prob_weights_right[i + 1] + 1

            # for i in range(num_iterations * batch_size):
            #     print(positions[predicted_idx][i].item(), end="\t")
            #     print(prob_weights_left[i].item(), end="\t")
            #     print(prob_weights_right[i])

            transitions_left_weight = (transitions_left[:, 0, 0] - 1/4) * (4/3)
            transitions_right_weight = (transitions_right[:, 0, 0] - 1/4) * (4/3)
            # transitions_left_weight = e ** (-20 * left_dist1 * num_generations)
            # transitions_right_weight = e ** (-20 * right_dist1 * num_generations)


            idx_expanded = torch.cumsum(predicted_idx.flip(0), 0).flip(0)
            prob_weights_left = torch.cat((prob_weights_left.flip(0), torch.ones((1,)).to(device)))
            left_weight = prob_weights_left[idx_expanded]
            transitions_left_weight[idx_expanded == 0] = 0

            idx_expanded = torch.cumsum(predicted_idx, 0)
            prob_weights_right = torch.cat((prob_weights_right, torch.ones((1,)).to(device)))  ### !
            right_weight = prob_weights_right[idx_expanded]
            transitions_right_weight[idx_expanded == 0] = 0


            middle_weight = torch.zeros((len_seq,)).float().to(device)
            middle_weight[predicted_idx] = 1

            total_weight = left_weight * transitions_left_weight + middle_weight + right_weight * transitions_right_weight

            if iteration == num_iterations - 1:
                for i in range(2000):
                    print(middle_weight[i].item(), end="\t")
                    print(left_weight[i].item(), end="\t")
                    print(transitions_left_weight[i].item(), end="\t")
                    print(right_weight[i].item(), end="\t")
                    print(transitions_right_weight[i].item(), end="\t")
                    print(total_weight[i].item())
                print("\n\n\n")
        ...
        #     for i in range(3):
        #         plt.plot(torch.arange(len_seq), predictions_last[:, i].cpu(), label=str(i))
        #     plt.legend()
        #     plt.show()
        ...
        #     print(None)
        # else:
        #     predictions_diff = (predictions - predictions_last).abs().sum().item()
        #     predictions_diff_strict = (predictions_last_strict != predictions_strict).sum().item()
        #     print("diff: ", predictions_diff, ", strict diff: ", predictions_diff_strict)


        predictions = predictions_last
        predictions_strict = predictions_last_strict
