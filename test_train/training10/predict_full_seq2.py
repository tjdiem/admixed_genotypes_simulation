...
def predict_full_sequence(model, SOI, positions, refs, labels, max_batch_size=batch_size, num_generations=None, population_size=None):
    ...
    for iteration in range(num_iterations):
        ...
        # out_smoothed[..., 1] *= 0.5
        # out_smoothed = out_smoothed / out_smoothed.sum(dim=-1, keepdim=True)
        # predictions += torch.log(out_smoothed).sum(dim=0)
        ...
        # ax1.imshow(predictions.unsqueeze(-1).expand(-1,-1,1000).reshape(-1,3000).cpu())
    ...
    positions_batch = positions[predicted_idx]
    positions_diff = (positions - positions_batch[:, input_size // 2].unsqueeze(-1)).abs() # batch, len_seq + input_size
