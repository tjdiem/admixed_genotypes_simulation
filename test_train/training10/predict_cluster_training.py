...
def predict_cluster(model, SOI, positions_bp, recombination_map, len_chrom_bp=None, batch_size=batch_size, num_generations=None, admixture_proportion=None, population_size=None, *, y_true=None):
    ...
    for i in range(0, num_individuals * len_seq, batch_size):
        ...
        # added!!!!!!!!!!!!!! 0000
        optimizer.zero_grad()
        switched = predictions[:, input_size // 2: -(input_size // 2)][y_true == 0][:, [0,2]].sum(dim=0) + predictions[:, input_size // 2: -(input_size // 2)][y_true == 2][:, [2,0]].sum(dim=0)

        if switched[-1] > switched[0]:
            out_true = 2 - y_true[ind3, ind4]

        else:
            out_true = y_true[ind3, ind4]

        out_pred = training_model(SOI_batch, refs_batch, labels_batch, positions_batch, params_batch)


        loss = criterion(out_pred, out_true.to(device))
...
# select random file just from training data
...
for _ in range(num_training_iterations):                                
    ...
    y_pred = predict_cluster(model, X, positions, recombination_map=recombination_map, batch_size=16, num_generations=num_generations, admixture_proportion=None, y_true=y)

    y_pred = y_pred.argmax(dim=-1)

    acc = max((y_pred == y).sum().item(), (2 - y_pred == y).sum().item()) / y.numel()
    print(f"{random_file}: {acc}")

    model = deepcopy(training_model)
