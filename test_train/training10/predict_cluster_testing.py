...
images = []
for i in range(7):
    for rseed in [202, 401]:
        with open(f"cluster_pred/y_pred{extra_title}_{file_num}_{rseed}_{i}", "r") as f:
            image = torch.tensor(eval(f.read()))

        if (image == 2).sum() > (image == 0).sum():
            image = 2 - image
...
pred = F.one_hot(images, num_classes=3).float() # 14, num_individuals, len_seq, 3
pred = torch.mean(pred, dim=0)
mask = pred.amax(dim=-1) < 1
pred[mask] = torch.tensor([0.25, 0.5, 0.25])

plt.imshow(pred, interpolation="nearest", aspect="auto")
plt.show()


@torch.no_grad()
def predict_cluster_tmp(model, SOI, positions_bp, recombination_map, len_chrom_bp=None, batch_size=batch_size, num_generations=None, admixture_proportion=None, population_size=None):
    ...
    for chunk in range(num_chunks):
        ...
        predictions_chunked[chunk] = pred.to(device)
    ...
    for chunk in range(0, num_chunks):
        ...
        for i in range(0, num_individuals * len_seq_chunked[chunk] * 2, batch_size):
...
t1 = time.time()
y_pred = predict_cluster_tmp(model, X, positions, recombination_map=recombination_map, batch_size=16, num_generations=num_generations, admixture_proportion=None)
print()
...
with open(f"cluster_pred/iter2_" + "_".join(sys.argv[1:]), "w") as f:
