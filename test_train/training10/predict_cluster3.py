...
"""
problem is very binary - accuracy is either between 50.25 and 51.25 or between 66 and 67
test if the problem goes away by adding more iterations.  
test the ancestry of infered position vs ancestry of first few infered positions
the problem might go away once NN is trained on this type of data

even when the problem doesnt occur, the accuracy on second chunk is significantly lower.
    why is this? could be because this chunk is just harder.  
    could be because of bug.  see if accuracy worsens further into the genome
"""
...
model.load_state_dict(torch.load("diverse_13_2.pth", map_location=torch.device(device)))
model.eval()
